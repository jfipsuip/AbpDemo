    ...
    public class AuditLog : Entity<long>
    {
        ...
        public static AuditLog CreateFromAuditInfo(AuditInfo auditInfo)
        {
            return new AuditLog
            {
                ...
                Exception = auditInfo.Exception?.ToString().TruncateWithPostfix(MaxExceptionLength),
        ...
        public override string ToString() => $"AUDIT LOG: {ServiceName}.{MethodName} is executed by user {UserId} in {ExecutionDuration} ms from {ClientIpAddress} IP address.";
    }
}
